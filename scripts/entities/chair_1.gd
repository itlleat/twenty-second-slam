...
func _ready():
	...
	if not is_projectile_mode:
		...
		if player:
			...
			print("Added collision exception with player for chair_1")
		...
		if has_node("HitBox"):
			...
			print("Warning: HitBox node not found for chair_1")
		...
		for child in parent.get_children():
			...
			if child != self and child is CharacterBody2D:
				if (child.name == "Enemy" or 
					...
					add_collision_exception_with(child)
					print("Chair_1 projectile added collision exception with: ", child.name)
...
func set_projectile_mode(enabled: bool):
	...
	if enabled:
		...
		print("Chair_1 set to projectile mode with velocity: ", velocity)
...
func _process(delta):
	...
	if is_flying:
		...
		if flying_timer <= 0:
			...
			print("Chair_1 came to rest and despawning")
...
func take_hit(damage: int = 1):
	health -= damage
	print("Chair_1 took hit! Health now: ", health)
	...
	if health <= 0:
		print("Chair_1 defeated at position: ", global_position)
